...
release:
	...
	@ docker-compose $(RELEASE_ARGS) up -d ntp
	@ $(call check_service_health,$(RELEASE_ARGS),ntp)
	${INFO} "Release environment created"
	${INFO} "NTP service is running - ntpdate -q $(DOCKER_HOST_IP)"
...
tag: 
	...
	@ $(foreach tag,$(TAGS),$(call tag_image,$(RELEASE_ARGS),ntp,$(DOCKER_REGISTRY)/$(ORG_NAME)/$(REPO_NAME):$(tag));)
	${INFO} "Tagging complete"

# Publishes image(s) tagged using make tag commands
publish:
	${INFO} "Publishing release image to $(DOCKER_REGISTRY)/$(ORG_NAME)/$(REPO_NAME)..."
	@ $(call publish_image,$(RELEASE_ARGS),ntp,$(DOCKER_REGISTRY)/$(ORG_NAME)/$(REPO_NAME))
